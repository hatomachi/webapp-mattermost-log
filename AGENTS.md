# AGENTS.md

## 概要
Mattermost の複数チーム・複数チャンネルをフラットに集約し、サーバーログ感覚で高密度に閲覧できるモバイル・PC対応 PWA です。

## 作戦ノートへの参照リンク
このプロジェクトのビジョン、現在地、Next Actions、フィードバックログは以下の作戦ノートで一元管理されています。
- **作戦ノート**: [/Users/s-ikari/work/personal-vault/10_職人・発明家/webapp-mattermost-log.md](/Users/s-ikari/work/personal-vault/10_職人・発明家/webapp-mattermost-log.md)
- **全体ダッシュボード**: [/Users/s-ikari/work/personal-vault/00_Dashboard.md](/Users/s-ikari/work/personal-vault/00_Dashboard.md)

作業時は常に上記作戦ノートと同期し、完了したタスクのチェックや現在地の更新を行ってください。
